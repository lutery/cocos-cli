...
import { queryVisualStudioVersion } from '../utils';

type Bundle = Record<string, unknown>;
type VisualStudioInfo = Awaited<ReturnType<typeof queryVisualStudioVersion>>;
...
let visualStudioCache: VisualStudioInfo | undefined;
...
export function activate(context: HostContext): void {
    ...
    context.registerMethod('queryVisualStudioVersion', async (refresh?: boolean) => {
        if (!refresh && visualStudioCache) {
            return visualStudioCache;
        }
        visualStudioCache = await queryVisualStudioVersion();
        return visualStudioCache;
    });
}
